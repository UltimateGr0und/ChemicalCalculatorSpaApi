...
const ElementComp = memo(function ElementComp({ element }: ElementCompProps) {
    ...
    const { elementOnClickAction } = useElementsActions();

    return (
        element.isActive ?
            <div className="relative size-25 bg-zinc-500 p-2 hover:bg-zinc-600 outline-4 outline-offset-2 outline-zinc-500" onClick={() => elementOnClickAction(element.atomicNumber)}>
            ...
            <div className={`relative size-25 p-2 m-1 ${GroupColor[element.elementType]}`} onClick={() => elementOnClickAction(element.atomicNumber)}>
                <h4 className="text-left text-3xl text-gray-200">{element.formula}</h4>
                <span className="absolute top-2 right-3 text-2xl text-gray-200">{element.atomicNumber}</span>
